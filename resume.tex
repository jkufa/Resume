\documentclass{resume} 
\usepackage{multicol}
\usepackage{hyperref}
\usepackage{tabularx}
\usepackage{hyperref}
\usepackage[dvipsnames]{xcolor}
\usepackage[left=0.75in,top=0.6in,right=0.75in,bottom=0.6in]{geometry}

\newcommand{\tab}[1]{\hspace{.2667\textwidth}\rlap{#1}}
\newcommand{\itab}[1]{\hspace{0em}\rlap{#1}}
\renewcommand{\baselinestretch}{.85}

\name{Jack H. Kufa}
% no idea why order has to be this way
% \address{(217) 827-5454 \\ jack@kufa.io}
\address{\href{https://www.kufa.io}{https://www.kufa.io} \\ \href{https://www.linkedin.com/in/jackkufa}{linkedin.com/in/jackkufa} \\ \href{https://www.github.com/jkufa}{github.com/jkufa}}
% \address{1869 White Columns Dr. \#304, Rolla MO} 

\begin{document}
\definecolor{myBlue}{RGB}{0,51,102}

% \begin{center}
%     1869 White Columns Dr. \#304, Rolla MO \\
%     (217) 827-5454 $\bullet$ jack@kufa.io \\
%     \url{https://www.kufa.io} $\bullet$ \href{https://www.linkedin.com/in/jackkufa}{linkedin.com/in/jackkufa} $\bullet$ \href{https://www.github.com/jkufa}{github.com/jkufa}
% \end{center}

% \begin{rSection}{Career Objective}
% To work for an organization which provides me the opportunity to improve my skills and knowledge.
% \end{rSection}

\begin{rSection}{Education}
    \small
    {
        \begin{tabular}{r|p{15cm}}
            \textsc{Fall} 2017 & Missouri University of Science and Technology, Rolla MO  \\ 
            \textsc{Fall} 2021 & \textbf{Anticipated B.S Computer Engineering \& B.S Computer Science} \\
                              & GPA: 3.48/4.00 \\
        \end{tabular}
    }
\end{rSection}

\begin{rSection}{Experience}
\small
{
  \begin{tabular}{r|p{15cm}}
    \textsc{May 2021} & \textbf{Service Management Group}, Kansas City MO \\
    \textsc{Aug 2021}  & \textit{Software Engineer Intern – Frontend Development} \\ &
    \begin{itemize}
      % \& wrote Confluence documentation
      \item {Designed reusable Angular components for internal UI design system }
      % Key Performance Indicators
      \item {Integrated KPI lite model backend functionality to improve loading times}
      \item {Refactored and corrected bugs in existing codebase to improve customer experience}
      \item {Developed Jasmine \& Postman unit tests to verify code functionality}
      \item {Coordinated with other developers and teams using Git TFS, Jira, \& Confluence}
    \end{itemize}
  \end{tabular}
  
  \begin{tabular}{r|p{15cm}}
    \textsc{May 2020} & \textbf{CinemaCraft LLC}, Remote \\
    \textsc{Aug 2020}  & \textit{Plugin Developer} \\ &
    \begin{itemize}
      \item {Interfaced with Spigot API to create custom Minecraft plugins}
      \item {Generated over 15.4 million YouTube views for content creators}
      \item {Deployed plugins to servers using Maven}
      \item {Managed servers remotely on Linux host}
      \item {Coordinated with other developers through weekly scrum meetings}
    \end{itemize}
  \end{tabular}

  \begin{tabular}{r|p{15cm}}
    \textsc{May 2019} & \textbf{Yaskawa America, Inc}, Oak Creek WI \\
    \textsc{Aug 2019}  & \textit{Software Engineer Intern – Full Stack Development} \\ &
    \begin{itemize}
        \item {Automated generation of product schematics}
        \item {Unprecedented integration of EPLAN API with Web Application}
        \item {Developed C\# ASP.NET MVC Web Application}
        \item {Designed SQL Database with Entity Framework \& Code First Migrations}
        \item {Corresponded with coworkers \& supervisor daily to update design specifications}
    \end{itemize}
  \end{tabular}
  
    \begin{tabular}{r|p{15cm}}
      \textsc{Aug 2017} & \textbf{Underwater Robotics Design Team}, Rolla MO \\
      \textsc{May 2020}  & \textit{Competition Lead} \\ &
      \begin{itemize}
          \item {Analyzed all competition rules regulations \& deadlines}
          \item {Implemented documentation for tracking travel deadlines \& necessities}
          \item {Managed documentation, trainings \& deadlines to ensure competition preparedness}
      \end{itemize}\\ &
      \textit{Vice President} \\ &
      \begin{itemize}
          \item {Secured new members through outreach \& promoted team growth}
          \item {Developed \& Implemented Discord bot using Discord's API to automate role assignment}
      \end{itemize}
    \end{tabular}
}
\end{rSection}


% Maybe this section could a brief description for each project??
\begin{rSection}{Projects}
    \small
    {
        \begin{tabular}{p{.3\textwidth} p{.35\textwidth} p{.3\textwidth} }% p{.25\textwidth}}
            % \textbf{Wellness Trainer} & \textbf{Doublecross} & \textbf{Controller Modding}\\
            \textbf{Doublecross} & 
            % \textbf{Drafty Discord Bot} & 
            \textbf{ShineGrab.tech}  & \textbf{FishTV Livestream}\\
            \textit{TigerHacks 2019} 
            % & \textit{Personal 2020} 
            & \textit{PickHacks 2021} & \textit{SMG Hackathon 2021} \\
            \begin{itemize}
              \item \textbf{Best use of Twilio award}
              \item Twilio API
              \item C\# ASP.NET
            \end{itemize} &
            % \begin{itemize}
            %     \item Pokemon Draft League
            %     \item SQLite3
            %     \item Python
            % \end{itemize} &
            \begin{itemize}
              \item \textbf{Top 5 Finish (Gaming Track)}
              \item Node \& Express
              \item Slippi-js
          \end{itemize} &
            \begin{itemize}
                \item Angular
                \item Socket.io
                \item 
            \end{itemize}

        \end{tabular}
    }
\end{rSection}

\begin{rSection}{Technical Skills}
    \small
    {
        \begin{tabular}{ @{} >{\bfseries}l @{\hspace{4ex}} l @{\hspace{4ex}} l @{\hspace{4ex}} l}
        \textcolor{white}{easily} & {\bf {Proficient}}& {\bf  {Familiar}} \\
        {\bf Languages} & Javascript, Typescript, HTML, CSS, C\#, Java, Python & C++\\
        {\bf Industry Knowledge} & Git, Linux, Bash/Zsh/Powershell, Agile, Scrum, Trello, Jira & REST APIs \\
        {\bf Tools \& Software} & Visual Studio, VS Code, IntelliJ, PuTTY, \LaTeX, Slack, Teams  & LINQ, Maven, Postman  \\
        \end{tabular}
    }
  \end{rSection}


% \begin{rSection}{Relevant Courses}
% \small
% {
%     \begin{tabular}{@{\hspace{0ex}} l @{\hspace{4ex}} l @{\hspace{4ex}} l @{\hspace{4ex}} l}
%     Data Structures & Digital Logic & Circuits I \& II & Intro to Electronic Devices\\
%     Operating Systems & Computer Organization \& Design & Microcontrollers
%     \end{tabular}
% }
% \end{rSection}



\end{document}